...
object ConfigTree {
  ...
  private def getAdminStep(vStep: OrientVertex, graph: OrientGraph, adminId: String): AdminConfigTreeStep = {
  ...
  private def getAdminComponents(vComponents: List[Vertex]): List[AdminConfigTreeComponent] = {
    vComponents.map({ vC => 
